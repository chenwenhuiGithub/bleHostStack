...
void QHci::send_cmd_le_remote_connection_parameter_request_reply(uint8_t* data) {
    uint8_t sendData[18] = { 0x00 };
    _assign_cmd(sendData, HCI_OGF_LE_CONTROLLER, HCI_OCF_LE_REMOTE_CONNECTION_PARAMETER_REQUEST_REPLY);
    sendData[3] = 14;
    memcpy_s(&sendData[4], 14, data, 14);
    serialPort.write((char*)sendData, sizeof(sendData));
    btsnoop.wirte(sendData, sizeof(sendData), BTSNOOP_DIRECT_HOST_TO_CONTROLLER);
}
...
void QHci::recv(uint8_t* data, uint16_t len)
{
    ...
    if (type == HCI_PACKET_TYPE_ACL) {
        ...
        qDebug("not supported HCI_PACKET_TYPE:%u", type);
    }
}
...
void QHci::recv_evt(uint8_t* data, uint16_t len)
{
    ...
    case HCI_EVENT_DISCONNECTION_COMPLETE:
        qDebug("disconnection_complete status:%u, connect_handle:0x%02x%02x, reason:0x%02x", data[2], data[3], (data[4] & 0x0f), data[5]);
    ...
    case HCI_EVENT_LE_META:
        recv_evt_le_meta(data + 1, len - 1);
        break;
    case HCI_EVENT_NUMBER_OF_COMPLETED_DATA_BLOCKS:
        break;
    default:
        qDebug("not supported HCI_EVENT:%u", event);
        break;
    }
}
...
void QHci::recv_evt_command_complete(uint8_t* data, uint16_t len)
{
    ...
    case HCI_OGF_INFORMATIONAL_PARAM:
        ...
        case HCI_OCF_READ_LOCAL_VERSION_INFO:
            qDebug("read_local_version_info status:%u, hci_version:%u", data[4], data[5]);
    ...
    case HCI_OGF_LE_CONTROLLER:
        ...
        break;
    default:
        qDebug("not supported HCI_OGF:%u", ogf);
        break;
    }
}

void QHci::recv_evt_le_meta(uint8_t* data, uint16_t len)
{
    (void)len;

    uint8_t sub_event = data[1];
    switch (sub_event) {
    case HCI_EVENT_LE_CONNECTION_COMPLETE:
        qDebug("le_connection_complete status:%u, connect_handle:0x%02x%02x, peer_address:%02x:%02x:%02x:%02x:%02x:%02x",
               data[2], data[3], (data[4] & 0x0f), data[7],  data[8], data[9], data[10], data[11], data[12]);
    ...
    case HCI_EVENT_LE_REMOTE_CONNECTION_PARAMETER_REQUEST:
        qDebug("le_remote_connection_parameter_request connect_handle:0x%02x%02x, interval_min:%0.2fms, interval_max:%0.2fms, max_latency:%u, timeout:%ums",
               data[2], (data[3] & 0x0f), (data[4] | (data[5] << 8)) * 1.25, (data[6] | (data[7] << 8)) * 1.25,
               data[8] | (data[9] << 8), (data[10] | (data[11] << 8)) * 10);
        // send_cmd_le_remote_connection_parameter_request_reply(reply);
        break;
    case HCI_EVENT_LE_ENHANCED_CONNECTION_COMPLETE:
        ...
        break;
    default:
        qDebug("not supported HCI_LE_EVENT:%u", sub_event);
        break;
    }
}
